...
namespace MyWMS.Helpers
{
    ...
    public static class AsyncCommand
    {
        ...
        {
            return new AsyncCommand<object>(async (o, _) => { await command(); return null; });
        }

        public static AsyncCommand<TResult> Create<TResult>(Func<Task<TResult>> command)
        {
            return new AsyncCommand<TResult>((o, _) => command());
        }

        public static AsyncCommand<object> Create(Func<CancellationToken, Task> command)
        {
            return new AsyncCommand<object>(async (_, token) => { await command(token); return null; });
        }
        public static AsyncCommand<TResult> Create<TResult>(Func<CancellationToken, Task<TResult>> command)
        {
            return new AsyncCommand<TResult>((_, token) => command(token));
        }

        public static AsyncCommand<object> Create(Func<object, CancellationToken, Task> command)
        {
            return new AsyncCommand<object>(async (o, token) => { await command(o, token); return null; });
        }
